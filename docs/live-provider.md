...
Calculated cost from provider usage is distinct from a reconciled billing charge. `actual_model_cost` remains unknown until real charges are reconciled; `calculated_model_cost` shows the known estimate. No silent fallback substitutes offline output for failed live inference. Standalone runs use per-run ceilings. The separately authorized [difficulty calibration](calibration-protocol.md) uses one shared campaign ledger, with atomic token, currency and call admission across both runs. Its fixed runner requires explicit live opt-in, validates the prepared source and protocol, and refuses any second execution after its persistent dispatch marker is created. It provides no automatic generation resume or general live comparison command.
