...
At the offline campaign revision, the implementation passed **63 tests**, including exhaustive oracle cross-checks at m=0..12, visibility separation, invalid claims, decimal price arithmetic, token subsets, concurrent reservations, missing usage, duplicate telemetry, retry attribution, restart recovery, sibling failure settlement, replay upper-bound checks and display projections. The offline quickstart and scripted replay were executed in a fresh Python 3.12 virtual environment without runtime packages. The actual Tk window was rendered, replayed and visually checked; it requires a graphical session and a Python build with Tk.

Subsequent secure credential and Terra accounting changes passed **89 tests**, with CI on Python 3.11, 3.12 and 3.13. The owner then authorized a three-call Terra solo pilot with USD 2 and 100,000-token total ceilings and no automatic retries. It completed using 2,260 tokens and USD 0.016160 calculated API cost, with a verified optimum at m=12. See the [separate live results record and preserved trace](terra-pilot.md). This pilot does not change the offline campaign's results or establish a coordination advantage.
...
The later [six-call Terra calibration](terra-calibration-results.md) selected m=24 under a precommitted rule: its worker's first and final sizes were 8 against exact optimum 10; m=18 was first-call optimal. The campaign used 10,395 tokens and USD 0.102580 calculated cost. These observations motivate a comparison on the existing task but do not demonstrate agent or coordination benefit.

## Separate live comparison — halted

The later [m=24 live comparison](live-comparison-results.md) started ten of twenty planned runs and halted on attempt 95 after a transport failure left usage unresolved. Five runs completed twelve calls; four failed earlier and one was interrupted. Their best valid sizes were eight or nine against exact optimum ten. Ten unstarted rows remain missing, not failures or zero-quality observations. Known usage totals 156,639 tokens and USD 1.499658 calculated cost, with an additional 28,075-token/USD 0.3076875 reservation held. These are incomplete API totals and are not billing-reconciled charges. This separate live evidence does not alter the historical offline results above.
